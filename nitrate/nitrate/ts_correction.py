"""
nitrate.ts_correction
======================
Temperature-, salinity-, and (optionally) pressure-corrected dissolved
nitrate concentration algorithms for SUNA/ISUS UV absorption spectra.

Functions
---------
plant2023_tsp_correction -- Plant et al. (2023) update to Sakamoto et al.
                             (2009), with the Sakamoto et al. (2017)
                             pressure correction. Used by
                             ``nitrate.calibration.add_plant_correction``.
sakamoto_2009             -- Original Sakamoto et al. (2009) algorithm, as
                             originally delivered by OOI. Retained for
                             comparison against the Plant (2023) result.
"""
...
def plant2023_tsp_correction(cal_temp, wl, eno3, eswa, di, dark_value, ctd_t,
                         ctd_sp, ctd_p, data_in, frame_type, wllower=217, wlupper=240):
    """
    Description:
    ...
    # coefficients to equation 6 of Plant et al 2023 that give the
    # temperature response as a function of wavelength
    c0 = 1.46380e-02
    c1 = 1.67660e-03
    c2 = 2.91898e-05
    c3 = -7.56395e-06
    ...
    for i in range(0, n_data_packets):
        ...
        if frame_type[i] == 'SDB' or frame_type[i] == 'SDF' or frame_type[i] == "NDF":
            ...
            # Now calculate the temperature response as a function of wavelength
            WL_prime = (WL - 210.0)
            f_prime = (c0 + c1 * WL_prime + c2 * WL_prime**2 + c3 * WL_prime**3
                       + c4 * WL_prime**4)

            # now estimate molar absorptivity of seasalt at in situ temperature
            # use Satlantic calibration and correct as in Plant et al 2023.
            SWA_Ext_at_T = (ESWA * np.exp(f_prime * (ctd_t[i] - cal_temp[i])))

            # next, adjust for the pressure effect from Sakamoto et al 2017
            if ctd_p is not None:
                SWA_Ext_at_T = SWA_Ext_at_T * (1 - ((0.026 * ctd_p[i]) / 1000))
...
def sakamoto_2009(cal_temp, wl, eno3, eswa, di, dark_value, ctd_t, ctd_sp,
                  data_in, frame_type, wllower=217, wlupper=240, ctd_p=None):
    """
    Description:
    ...
    for i in range(0, n_data_packets):
        ...
        if frame_type[i] == 'SDB' or frame_type[i] == 'SDF' or frame_type[i] == "NDF":
            ...
            if ctd_p is not None:
                # Add in pressure corrections from Sakamoto et al 2017
                SWA_Ext_at_T = SWA_Ext_at_T * (1 - ((0.026 * ctd_p[i]) / 1000))
